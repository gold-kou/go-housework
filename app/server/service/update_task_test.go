...
func TestUpdateTask_Execute(t *testing.T) {
	type args struct {
		auth       *model.Auth
		updateTask *schemamodel.RequestUpdateTask
	}
	tests := []struct {
		name                     string
	...
	}{
		{
			...
			mockTaskRepoFunc: func(r *repository.MockTaskRepositoryInterface) {
				r.EXPECT().UpdateTask(&db.Task{ID: common.TestTaskID1, Name: common.TestTaskName1, MemberID: common.TestUserID, FamilyID: common.TestFamilyID, Status: common.TestTaskStatusTodo, Date: common.TestTaskDate}).Return(nil)
			},
			args: args{
				auth:       &model.Auth{UserName: common.TestUserName},
				updateTask: &schemamodel.RequestUpdateTask{Task: schemamodel.Task{TaskId: int64(common.TestTaskID1), TaskName: common.TestTaskName1, MemberName: common.TestUserName, Status: common.TestTaskStatusTodo, Date: common.TestTaskDate}},
			},
			want1:   &db.Task{ID: common.TestTaskID1, Name: common.TestTaskName1, MemberID: common.TestUserID, FamilyID: common.TestFamilyID, Status: common.TestTaskStatusTodo, Date: common.TestTaskDate},
			want2:   &db.Family{ID: common.TestFamilyID, Name: common.TestFamilyName},
			want3:   &db.User{ID: common.TestUserID, Name: common.TestUserName},
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			common.MockDB(t, func(db *gorm.DB, mock sqlmock.Sqlmock) {
				...
				// run target method
				got1, got2, got3, err := NewUpdateTask(userRepo, familyRepo, memberFamilyRepo, taskRepo).Execute(tt.args.auth, tt.args.updateTask)
